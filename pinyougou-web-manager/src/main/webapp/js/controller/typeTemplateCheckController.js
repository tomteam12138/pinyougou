...
app.controller('typeTemplateCheckController' ,function($scope,$controller,typeTemplateCheckService){
	...
	$scope.findAll=function(){
        typeTemplateCheckService.findAll().success(
			function(response){
				$scope.list=response;
			}			
		);
	...
	$scope.findOne=function(id){
        typeTemplateCheckService.findOne(id).success(
			function(response){
	...
	$scope.save=function(){				
		...
		if($scope.entity.id!=null){//如果有ID
			serviceObject=typeTemplateCheckService.update( $scope.entity ); //修改
		}else{
			serviceObject=typeTemplateCheckService.add( $scope.entity  );//增加
		}				
	...
	$scope.search=function(page,rows){
        typeTemplateCheckService.search(page,rows,$scope.searchEntity).success(
			function(response){
				$scope.list=response.rows;	
				$scope.paginationConf.totalItems=response.total;//更新总记录数
			}			
